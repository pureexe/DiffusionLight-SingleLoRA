...
def create_argparser():    
    ...
    parser.add_argument("--algorithm", type=str, default="special", choices=["iterative", "normal", "special"], help="Selecting between iterative or normal (single pass inpaint) algorithm")
...
def main():
    ...
    os.chmod(raw_output_dir, 0o777)
    os.chmod(square_output_dir, 0o777)
    ...
    for image_id, image_data in tqdm(enumerate(dataset), total=len(dataset)):
        ...
        for ev, (prompt_embeds, pooled_prompt_embeds) in embedding_dict.items():
            ...
            for seed in seeds:
                ...
                elif args.algorithm == "special":
                    output_image = pipe.inpaint_special(**kwargs).images[0]
                else:
                ...
                # save image 
                output_image_path = os.path.join(raw_output_dir, outpng)
                square_image_path = os.path.join(square_output_dir, outpng)
                output_image.save(output_image_path)
                square_image.save(square_image_path)
                os.chmod(output_image_path, 0o777)
                os.chmod(square_image_path, 0o777)
