...
from relighting.pipeline_utils import (
    custom_prepare_latents,
    custom_prepare_mask_latents,
    rescale_noise_cfg,
    predict_x0
)
...
class CustomStableDiffusionXLControlNetInpaintPipeline(StableDiffusionXLControlNetInpaintPipeline):
    ...
    def get_pred_x0(
        ...
        assert latent_timestep is not None
        ...
        self.predict_x0 = predict_x0.__get__(self, CustomStableDiffusionXLControlNetInpaintPipeline)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate([latent_timestep]):
                ...
                # estimated x0 (work for unipc only)
                if use_predict_x0_latent:
                    predict_x0_latents = self.predict_x0(latents, noise_pred, int(t))
    ...
    def __call__(
        ...
        use_noise_moving=True,
        use_predict_x0_latent=False,
        switch_lora_during_denoise=False,
            ...
            for i, t in enumerate(timesteps):
                ...
                if switch_lora_during_denoise and (not switch_lora_flag):
                    if t.item() < switch_lora_timestep:
                        self.switch_lora(switch_lora_path, switch_lora_scale)
                        switch_lora_flag = True
                ...
                # estimated x0 (work for unipc only)
                if use_predict_x0_latent:
                    predict_x0_latents = self.predict_x0(latents, noise_pred, t)
