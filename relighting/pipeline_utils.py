...
def predict_x0(self, latents, noise_pred, t):
    if isinstance(self.scheduler, UniPCMultistepScheduler):
        alpha_t, sigma_t = self.scheduler.alpha_t[t], self.scheduler.sigma_t[t]
        x0_pred = (latents - sigma_t * noise_pred) / alpha_t
    else:
        raise NotImplementedError("enable_acceleration is currently not supported for other schedulers")
