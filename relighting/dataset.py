...
def filter_non_empty_files(file_paths):
    """
    Filters out files with a size of 0 bytes from the given list of file paths.

    Args:
        file_paths (list of str): List of file paths to filter.

    Returns:
        list of str: List of file paths that have a size greater than 0 bytes.
    """
    return [file for file in file_paths if os.path.isfile(file) and os.path.getsize(file) > 0]
...
class Dataset(ABC):
    ...
    def __getitem__(self, idx):
        image = Image.open(self.scene_data[idx]).convert("RGB")
...
class GeneralLoader(Dataset):
    def __init__(self,
        ...
        if self.root is not None:
            ...
            paths = filter_non_empty_files(paths)
