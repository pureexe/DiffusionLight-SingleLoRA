...
INPUT_SHADING_DIR = "/ist/ist-share/vision/pakkapon/relight/DiffusionLight-SingleLoRA/output/multi_illumination/least_square/train/{}/shading_exr_perspective_v3_order6_marigold"
OUTPUT_DIR = "/ist/ist-share/vision/relight/datasets/multi_illumination/shadings/least_square/v3/train"
...
def process_scene(scene_name):
    ...
    shading_files = [a.replace('.exr','') for a in os.listdir(input_shading_dir)]
    # find intersect 
    ...
    output_shading_dir = os.path.join(OUTPUT_DIR, "shadings_marigold", scene_name)
...
def main():
    ...
    with Pool(THREAD) as p:
        results = list(tqdm(p.imap(process_scene, scenes), total=len(range(START_FRAME, TOTAL_SCENE))))
        # exit()
        # indexs = {
        #     "image_index": [],
        #     "envmap_index": []
