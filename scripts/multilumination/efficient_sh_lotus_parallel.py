...
def process_scene(args, info):
    ...
    output_dir = os.path.join(args.output_dir_template, scene_name)
    ...
    normal_dir = os.path.join(args.normal_dir_template, scene_name)        
    ...
    if args.use_viz==1:
        vizmax_dir = os.path.join(args.vizmax_dir_template, scene_name)
        vizldr_dir = os.path.join(args.vizldr_dir_template, scene_name)
...
def efficient_rendering(args):
    queues = []
    scenes = sorted(os.listdir(args.normal_dir_template))
    for scene_name in tqdm(scenes[args.index::args.total]):
        coeff_dir = os.path.join(args.coeff_dir_template, scene_name)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--coeff_dir_template', type=str, default="/ist/ist-share/vision/relight/datasets/multi_illumination/spherical/train/shcoeff_perspective_order100", help='template for coeff dir')
    parser.add_argument('--normal_dir_template', type=str, default="/ist/ist-share/vision/relight/datasets/multi_illumination/spherical/train/normal_lotus", help='template for normal dir')
    parser.add_argument('--output_dir_template', type=str, default="/ist/ist-share/vision/relight/datasets/multi_illumination/spherical/train/shading_exr_perspective_fov_order6", help='template for output dir')
    parser.add_argument('--vizmax_dir_template', type=str, default="/ist/ist-share/vision/relight/datasets/multi_illumination/spherical/train/shading_exr_perspective_fov_order6_viz_max", help='template for vizmax dir')
    parser.add_argument('--vizldr_dir_template', type=str, default="/ist/ist-share/vision/relight/datasets/multi_illumination/spherical/train/shading_exr_perspective_fov_order6_viz_ldr", help='template for vizldr dir')
